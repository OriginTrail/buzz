...
async fn finalize_dkg_memory_after_success(
    ...
    let (Some(schema), PromptSource::Channel(scope)) = (ctx.dkg_memory_schema, source) else {
        return;
    };
    // Agent sessions may now be scoped to a thread, but DKG authorization and
    // Context Graph binding remain channel-scoped. Every thread in a Buzz
    // channel therefore contributes to that channel's graph.
